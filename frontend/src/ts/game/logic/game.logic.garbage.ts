...
export function getGarbageAmount(clearAmount: number, combo: number, perfectClear: boolean): number {
    let garbageAmount = 0;
    if (clearAmount === 3) {
        garbageAmount += Math.floor(combo / 2);
    }
    if (clearAmount === 4) {
        garbageAmount += Math.ceil(Math.floor(combo / 2) + combo + 2);
    }
    if (clearAmount >= 5) {
        garbageAmount += combo + 2;
    }
    if (perfectClear) {
        garbageAmount += 7
    }
