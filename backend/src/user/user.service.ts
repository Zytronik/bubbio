...
export class UserService {
    ...
    async getUserDetailsById(userId: number): Promise<UserDetails> {
        ...
        const rankInfos: RankInfo = await this.ranksService.getRankInfo(
            user.id,
        );
    ...
    async getPercentile(userId: number): Promise<number> {
        ...
        return (
            Math.round(((rank / totalUsers) * 100 + Number.EPSILON) * 100) / 100
        );
    }
    ...
    async getPercentiles(
        ...
        userIds.forEach(userId => {
            ...
            const percentile =
                Math.round(
                    ((rank / rankedUsers) * 100 + Number.EPSILON) * 100,
                ) / 100;
